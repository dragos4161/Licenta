...
class _UserChatPageState extends State<UserChatPage> {
  Future<void> checkRequestAccepted() async {
    final String uid = StoreProvider.of<AppState>(context).state.auth.user!.uid;
    final DocumentSnapshot<Map<String, dynamic>> ref =
        await FirebaseFirestore.instance.collection('queue').doc(uid).get();

    final String status = ref.data()!['status'].toString();
    if (status == 'pending') {
  ...
  late bool requestAccepted = false;
      ...
      builder: (BuildContext context, AppUser? user) {
        return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot<Map<String, dynamic>>> snapshot) {
            ...
            if (snapshot.data!.data()!['status'] == 'pending' || snapshot.data!.data()!['status'] == 'closed') {
              return Scaffold(
                  ...
                  children: <Widget>[
                      ...
                      child: const DecoratedBox(
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage('assets/images/waitingroom.jpg'),
                          ),
                        ),
                      ),
                    ),
                    Column(
                      children: <Widget>[
                        ...
                        Center(child: LoadingAnimationWidget.waveDots(color: Colors.blueAccent, size: 50)),
                        Center(
                            ...
                            showDialog<String>(
                              ...
                              builder: (BuildContext context) => AlertDialog(
                                ...
                                actions: <Widget>[
                                  TextButton(
                                    onPressed: () {
                                      FirebaseFirestore.instance.collection('queue').doc(user.uid).update(
